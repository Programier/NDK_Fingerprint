...
#define STD_INFO "std_version: 3.0 beta6, build time: 2016-09-30/10:57"
#define MTAG 	"MTAG"
...
//debug
//struct timeval gtv1, gtv2,gtv3,gtv4;
...
int ma_enroll(const hw_auth_token_t *hat, int gid, int timeout) {
	...
	dprint(MTAG, "%s: my####### start. gid=%d timeout=%d\n", __func__, gid, timeout);

	smaf.hat = *hat;
	dprint(MTAG, "%s: dbOpen. ret=%d\n", __func__, ret);
...
int ma_cancel() {
	...
	dprint(MTAG, "%s: my###### end. i=%d ret=%d\n", __func__, i, ret);
...
int ma_verify(uint64 operation_id, int gid) {
	...
	dprint(MTAG, "%s: my###### start. oper_id=0x%llx gid=%d\n", __func__, operation_id, gid);
	...
	if(smaf.fdev!=-1 && smaf.fdb!=-1) {
		...
		if (what != DO_DEBUG) {
			...
			smaf.stop = FALSE;//what this means?
			smaf.gid = gid;
			smaf.first = 0;//what this means?
			dprint(MTAG, "%s: my######===============================================\n", __func__);
			fp_setWork(DO_MATCH);
		}
	}
...
static int check(int timeout) {
	...
	fp_initCheck();
	dprint(MTAG, "%s my##### after fp_initCheck: ret=%d\n", __func__, ret);
	while(gap<timeout) {
		int what = fp_getWork();
		...
		//dprint(MTAG, "check #########=========================================================================\n");
		ret = fp_getImage();
		//dprint(MTAG, "%s ######### fp_getImage: ret=%d\n", __func__, ret);
		if(ret!=FP_OK) break;

		ret = fp_check();
		//gettimeofday(&tv3, NULL);
		//time_ms_check = ((tv3.tv_sec - tv1.tv_sec) * 1000000 + (tv3.tv_usec - tv1.tv_usec)) / 1000;
		//dprint(MTAG, "%s my##### time_ms_fp_check=%d,ret=%d\n", __func__,time_ms_check,ret);
		dprint(MTAG, "%s my##### fp_check: ret=%d\n", __func__, ret);
		if(ret==FP_CHK_UP) { //抬起
			dprint(MTAG, "%s: finger up.\n", __func__);
			//if(what==DO_IDLE || repo==1) fp_setValue(DRV_KEY_UP, 0);
			what = fp_getWork();
			dprint(MTAG, "now1 what is: %d",what);
			if(what==DO_IDLE) {
				dprint(MTAG, "my###### DRV_KEY_UP what=%d",what);
				dprint(MTAG, "my key report###### DRV_KEY_UP what=%d",what);
				fp_setCmd(DRV_KEY_UP, 0);
			}
			dprint(MTAG, "my######22222 DRV_KEY_UP",what);
			smaf.match = 0;
			break;
		} else if(ret==FP_CHK_FULL || ret==FP_CHK_PART) { //全部、部分接触
//			gettimeofday(&tv3, NULL);
//			time_ms_check = ((tv3.tv_sec - tv1.tv_sec) * 1000000 + (tv3.tv_usec - tv1.tv_usec)) / 1000;
//			dprint(MTAG, "%s my##### time_ms_check=%d,ret==FP_CHK_FULL || ret==FP_CHK_PART\n", __func__,time_ms_check);
			dprint(MTAG, "%s my##### ret==FP_CHK_FULL || ret==FP_CHK_PART\n", __func__);
			break;
		} else if(ret==FP_CHK_DOWN && !fdown) { //按压
			dprint(MTAG, "%s: finger down.\n", __func__);
			//if(what==DO_IDLE || repo==1) fp_setValue(DRV_KEY_DOWN, 0);
			what = fp_getWork();
			dprint(MTAG, "now2 what is: %d",what);
			if(what==DO_IDLE) {
				dprint(MTAG, "my######11111 FP_CHK_DOWN what=%d",what);
				dprint(MTAG, "my key report###### FP_CHK_DOWN what=%d",what);
				fp_setCmd(DRV_KEY_DOWN, 0);
			}
			dprint(MTAG, "my######22222 FP_CHK_DOWN",what);
			fdown = 1;
			timeout += 300;	//延时
		}
...
static int enroll(int val) {
	...
	notify_msg(FINGERPRINT_ACQUIRED, 0);//解决录入过程中系统黑屏的问题
	...
	if(val==FP_CHK_PART) { //部分接触
		...
		dprint(MTAG, "%s: #my grade #####. grade=%d,smaf.grade=%d\n", __func__, grade,smaf.grade);
...
static int match(int val) {
	...
	int id, ret, cnt=0;
	int gm = fp_check();
	dprint(MTAG, "%s my#####: start.gm val=%d\n", __func__, gm);
	if(smaf.match == 1){
		dprint(MTAG, "%s my#####: gm match refuse.gm val=%d\n", __func__);
	  return -1;	
	}
	smaf.match = 1;
	...
	for(id=1; id<=MAXF; id++) {
		...
		dprint(MTAG, "%s ----my#####: fp_match. ret=%d,sfpc.level=%d\n", __func__, ret,sfpc.level);
	...
	if(suc>0) { //匹配成功
		dprint(MTAG, "%s ----my#####: fp_match successfully.\n", __func__);
		notify_match(smaf.gid, suc);
		fp_dbUpdate(suc);
		//fp_setWork(DO_IDLE);//add by gm 
	} else if(cnt<MAXF) { //匹配失败
		dprint(MTAG, "%s ----my#####: fp_match failed. \n", __func__);
		notify_match(smaf.gid, 0);
	}

	dprint(MTAG, "%s: my##### end.\n", __func__);
...
static void interrupt_event(int what) {
	...
	if(what==DO_IDLE && repo==1) { //应用上报事件
		...
		// ########
		dprint(MTAG, "%s #########: start enroll sys i=%d check ret=%d", __func__, i,  ret);
		for(i=0, j=0; i<20; i++) {
			ret = check(300);
			...
			if(ret==FP_COMM_FAIL) { //通讯失败
				...
				if(what==DO_ENROLL) {
					dprint(MTAG, "%s ######### DO_ENROLL (check return): ret = %d ", __func__,ret);
					enroll(ret);
				}
				else if(what==DO_MATCH){
					dprint(MTAG, "%s ######### DO_MATCH (check return): ret = %d ", __func__,ret);
					if(smaf.match!=1)
					match(ret);
				}
	...
	fp_detectParam();//spend about 400ms?
	fp_detectMode();
	fp_setValue(DRV_WAKE_UNLOCK, 0);
	//gettimeofday(&gtv4, NULL);
	//this time_ms4 is about 400ms,why????
	//dprint(MTAG, "%s my##### time_ms4: ret = %d, time_ms4=%d ", __func__,ret,time_ms4);
	dprint(MTAG, "%s: end.\n", __func__);
}
